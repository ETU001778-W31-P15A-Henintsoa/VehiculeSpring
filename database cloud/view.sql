create or replace view v_venteVoiture as
    select  v.idVente,v.dateVente,v.idVendeur,v.idMpividy,v.prix,a.couleur,a.prix as prixAnnonce,a.prixminimum,a.dateannonce,a.estvendue,sm.*,m.idmarque,m.idcategorie
        from Vente v
...
create or replace view venteVoiture as
    select v.* ,  extract(year from v.dateVente) as annee,extract(month from v.dateVente) as mois
...
create or replace view meilleurVendeur as 
    select mv.*,nomutilisateur from v_meilleurVendeur mv
